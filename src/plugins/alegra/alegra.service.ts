...
export class AlegraService {
  ...
  async sendInvoice(ctx: RequestContext, order: Order): Promise<any> {
    if (!this.options.email || !this.options.token) {
      Logger.warn(
        'Alegra credentials not configured. Skipping invoice creation.',
        this.loggerCtx
      );
      return null;
    }
    ...
    try {
      ...
      Logger.info(
        `Sending invoice to Alegra for order ${order.code}`,
        this.loggerCtx
      );
      ...
      Logger.info(
        `Invoice created successfully in Alegra for order ${order.code}. Invoice ID: ${response.data.id}`,
        this.loggerCtx
      );
      ...
      let errorMessage = `Failed to send invoice to Alegra for order ${order.code}: ${error.message}`;

      // Mensajes  descriptivos según el código de estado
      if (statusCode === 402) {
        errorMessage = `Failed to send invoice to Alegra for order ${order.code}: Payment Required (402). Your Alegra account may need an active subscription plan or has reached the free plan limit.`;
      } else if (statusCode === 401) {
        errorMessage = `Failed to send invoice to Alegra for order ${order.code}: Unauthorized (401). Please check your ALEGRA_EMAIL and ALEGRA_TOKEN credentials.`;
      } else if (statusCode === 400) {
        errorMessage = `Failed to send invoice to Alegra for order ${order.code}: Bad Request (400). The invoice data may be invalid. Check the order data.`;
      }
  ...
  private buildInvoicePayload(order: Order): any {
    ...
    const invoicePayload: any = {
      ...
      client: {
        ...
        identification:
          (customer?.customFields as any)?.identification ||
